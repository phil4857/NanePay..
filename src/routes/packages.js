...
router.get('/', async (req, res) => {
  try {
    ...
    if (merchantId) {
      query = query.where('o.merchant_id', merchantId)
    }

    if (type) {
      query = query.where('o.duration_type', type)
    }
    ...
    logger.error('Get packages failed', {
      error: err.message,
    })
...
router.get('/:id', async (req, res) => {
  try {
    ...
    logger.error('Get package failed', {
      error: err.message,
    })
    ...
    try {
      ...
      if (!merchant) {
        return res.status(403).json({
          error: 'Merchant account not approved yet',
        })
      }
      ...
      if (!name || !durationType || !durationHours || !price) {
        return res.status(400).json({
          error: 'name, durationType, durationHours and price are required',
        })
      }
      ...
      if (!validTypes.includes(durationType)) {
        return res.status(400).json({
          error: `durationType must be one of: ${validTypes.join(', ')}`,
        })
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Delete package failed', {
        error: err.message,
      })
